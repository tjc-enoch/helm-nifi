    ...
    describe('NiFi Login via OIDC', () => {
        ...
        it('NiFi redirects to KeyCloak login page', async () => {
            ...
            for (let i = 0; i < 60 && !gotoSucceeded; i++) {
                try {
                    ...
                    console.log(`Connection to ${process.env.NIFIURL} failed: ${err.message} (try ${i})`);
        ...
        it('nifi@example.com shown as logged in user', async () => {
            ...
            for (let i = 0; i < 30 && messageContent !== ""; i++) {
                ...
                if (messageContent !== "") {
                    console.log(`Message Content: ${messageContent} (try ${i})`);
                }
            }
        ...
        it('All nodes connected', async () => {
            let connectedNodesUserElement = await page.waitForSelector('span[id="connected-nodes-count"]');
            let connectedNodesCount = await connectedNodesUserElement.evaluate(el => el.textContent);

            for (let i = 0; i < 30 && connectedNodesCount !== "3 / 3"; i++) {
                console.log(`Connected Nodes Count: ${connectedNodesCount} (try ${i})`);
                await Promise.all([
                    page.reload(),
                    page.waitForNavigation(),
                    page.waitForNetworkIdle(),
